...
\addbibresource{report_2.bib} %Import the bibliography file
...
\title{
  \textbf{Nexium} \\
  \large Rapport de soutenance n°2}
...
\fancyhf[HR]{\textbf{Rapport de soutenance n°2}}
...
Nous aborderons dans ce rapport la suite des \textbf{avancées} que nous avons pu réaliser depuis le début du projet. Les notions techniques les plus importantes du projet seront explorées, et nous détaillerons également la situation globale du groupe, de son organisation, et des perspectives à venir.
...
Étant moi même passionné de cryptographie, c'est tout naturellement que j'ai initialement suggéré à mon groupe l'idée d'une cryptomonnaie. Accompagnée par un \textit{synopsis} détaillant les grandes lignes théoriques du projet, l'idée a plu au groupe et a évolué au fil de plusieurs réunions et débats.
...
\begin{itemize}
    \item \textbf{En-tête du bloc} (\textit{82 octets})
    \begin{itemize}
        ...
        \item \texttt{difficulty\_target} (\textit{4 octets}) : \textbf{Cible de difficulté} du bloc.
        \item \texttt{nonce} (\textit{4 octets}) : \textbf{Nonce} du bloc.
        \item \texttt{transaction\_size} (\textit{4 octets}) : \textbf{Taille} en octets des transactions du bloc.
    \end{itemize}
    \item \textbf{Transactions} (\textit{variable}) : \textbf{Liste} des transactions.
    \begin{itemize}
        \item \texttt{transaction\_header} (\textit{73 octets}) : \textbf{En-tête} de la transaction.
        \begin{itemize}
            \item \texttt{transaction\_size} (\textit{4 octets}) : \textbf{Taille} de la transaction.
            \item \texttt{timestamp} (\textit{4 octets}) : \textbf{Horodatage} de la transaction.
            \item \texttt{fees} (\textit{2 octets}) : \textbf{Frais} de la transaction.
            \item \texttt{emitter} (\textit{64 octets}) : \textbf{Login} de l'émetteur de la transaction. (\texttt{prenom.nom}).
            \item \texttt{data\_type} (\textit{1 octet}) : \textbf{Type} de la transaction.
        \end{itemize}
        \item \texttt{data} (\textit{de 1 à 1719 octets}) : \textbf{Données} de la transaction.
        \item \texttt{signature} (\textit{256 octets}) : \textbf{Signature} de la transaction.
...
Nous pouvons ainsi, par un système dont nous avons théorisé les bases, accéder à la \textbf{clé publique} de n'importe quel utilisateur via l'\textbf{API de Gitlab} \cite{gitlabgpg}. La requête suivante permet de récupérer la clé publique d'un \texttt{login} donné : 

\begin{center}
\boxed{\text{GET} /\texttt{login}/gpg\_keys/key\_id}
\end{center}
...
Ces types de taille fixe en mémoire ont donc posé problème pour la génération. Pour résoudre ce problème, nous avons dû utiliser les crates \href{https://crates.io/crates/num-bigint}{\textcolor{black}{\texttt{num-bigint}}} et \href{https://crates.io/crates/num-primes}{\textcolor{black}{\texttt{num-primes}}}.


Ces \textbf{crates} nous permettent de générer de grands entiers sans limite de taille. Par exemple, \texttt{num\_bigint::BigUint} est une structure dynamique qui s’adapte à la taille du nombre, contrairement aux types natifs de Rust (grâce à l’utilisation de vecteurs \texttt{u32} ou \texttt{u64} pour limiter la taille en mémoire). Nous avons également utilisé \texttt{num\_primes} pour générer aléatoirement et manipuler des nombres premiers efficacement.
...
\paragraph{Signature (\texttt{sign})}~

La première fonction, \texttt{sign}, permet de générer une signature numérique pour un message donné. Elle repose sur le principe selon lequel seul le détenteur de la clé privée doit être capable de produire une signature valide. Voici le déroulé de l'opération :

\begin{itemize}
  \item Le message est tout d'abord transformé en un condensé (hash) grâce à l’algorithme SHA-256 implémenté plus tôt dans le projet. Ce hachage garantit que la signature porte uniquement sur le contenu du message, indépendamment de sa taille.
  \item Le condensé est ensuite converti en un entier (\texttt{BigUint}) pour être manipulé par les fonctions RSA.
  \item L’entier est signé en le mettant à la puissance $d$ (exposant privé) modulo $n$ :
\[
S = H(m)^d \mod n
\]
  \item La fonction renvoie une erreur si le message est vide ou si la taille du haché dépasse celle du module $n$.
\end{itemize}

Cette signature peut ensuite être transmise avec le message pour vérification.

\paragraph{Vérification de signature (\texttt{check\_signature})}~

La fonction \texttt{check\_signature} permet de s’assurer que la signature reçue correspond bien au message. Elle utilise la clé publique du signataire, ce qui garantit que seule la personne possédant la clé privée a pu produire cette signature.

\begin{itemize}
  \item Le message reçu est haché de la même manière que dans la fonction de signature.
  \item La signature est déchiffrée en la mettant à la puissance $e$ (exposant public) modulo $n$ :
\[
m' = s^e \mod n
\]
  \item Si le résultat du déchiffrement correspond au hash recalculé, alors la signature est considérée comme valide.
\end{itemize}
...
\paragraph{Chiffrement (\texttt{crypt})}~

La fonction \texttt{crypt} permet de chiffrer un message de manière confidentielle. Ce chiffrement est réalisé à l’aide de la clé publique du destinataire.

\begin{itemize}
  \item Le message est tout d'abord converti en un entier.
  \item Il est ensuite chiffré par l’opération RSA classique :
\[
c = m^e \mod n
\]
  \item Une vérification est effectuée pour s’assurer que la valeur à chiffrer est inférieure au module $n$, sinon une erreur est renvoyée.
\end{itemize}
...
\paragraph{Déchiffrement (\texttt{decrypt})}~
...
m = c^d \mod n
\]
  \item On obtient ainsi le message original, à condition que la clé privée utilisée corresponde à la clé publique ayant servi au chiffrement.
  \item Comme toujours, si le message est vide ou trop grand, une erreur est renvoyée.
\end{itemize}
...
\paragraph{Paquet de clé publique}~

En premier lieu, nous construisons un paquet de clé publique contenant :
\begin{itemize}
  \item La version du format OpenPGP (4) ;
  \item Un \textit{timestamp} correspondant à la date de création ;
  \item L’identifiant de l’algorithme (1 pour RSA) ;
  \item Les deux entiers principaux de la clé RSA : $n$ et $e$.
\end{itemize}

Ces derniers sont encodés au format MPI (\textit{Multi Precision Integer}) :
\begin{itemize}
  \item Une entête de deux octets encode la longueur de l’entier en bits ;
  \item Une séquence d’octets représente la valeur de l’entier en \textit{Big Endian}.
\end{itemize}
...
\paragraph{Paquet de clé privée}~
...
Ce chiffrement utilise un mécanisme appelé S2K (\textit{String to Key}) qui dérive une clé AES à partir :
\begin{itemize}
  \item du mot de passe ;
  \item d’un \textit{salt} de 8 octets ;
  \item d’un nombre d’itérations ;
  \item d’un vecteur d’initialisation aléatoire.
\end{itemize}
...
\paragraph{Paquet d'identifiant utilisateur}~
...
\begin{itemize}
  \item d’un en-tête ;
  \item de sous-paquets hachés et non hachés ;
  \item d’une empreinte SHA-256 ;
  \item d’une structure ASN.1 suivie d’une signature RSA faite avec la clé privée.
\end{itemize}

Comme pour les autres entiers cryptographiques, cette signature est encodée au format MPI.

\paragraph{Encapsulation PEM}~

Une fois tous les paquets assemblés pour la clé publique (ou la clé privée), ils sont :
\begin{itemize}
  \item concaténés en binaire ;
  \item encodés en base64 ;
  \item complétés par un \texttt{CRC24} (contrôle d'intégrité sur 3 octets).
\end{itemize}
...
Le \textbf{site web officiel de Nexium} est disponible à l'adresse \url{https://jean.herail.pages.epita.fr/nexium}

Afin d'illustrer la \textbf{facilité d'utilisation} et de développement propre à \texttt{Astro}, voici un exemple de code \texttt{Markdown} suffisant à la génération d'une page Web: 
...
Nous stockons la **blockchain** dans un fichier binaire `nxm_blckchn.dat`.

\end{lstlisting}

\textbf{Astro} se charge de convertir ce code \texttt{Markdown} en \texttt{HTML} statique, prêt à être servi par un serveur web. C'est ensuite \textbf{GitLab Pages} \cite{gitlabpages} qui se charge du \textbf{build}, et de l'\textbf{exposition} automatique du site à chaque nouveau \texttt{git push}.


\newpage

\section{Gitlab}
Dans le cadre de notre projet, nous avons développé une API Gitlab en Rust, afin d'interfacer facilement notre application avec l'instance Gitlab d'Épita (\href{https://gitlab.cri.epita.fr/}{gitlab.cri.epita.fr}). Cette API maison encapsule plusieurs fonctionnalités essentielles, telles que :

\begin{itemize}
    \item l'authentification OAuth2 (via le flux d'autorisation PKCE),
    \item la vérification de la validité d'un jeton d'accès,
    \item la récupération des identifiants utilisateurs et de leurs clés GPG,
    \item l'ajout d'une clé GPG à un compte utilisateur.
\end{itemize}
...
Pour les clients, l'authentification se fait via un navigateur web ouvert localement. Une fois l'utilisateur connecté, un petit serveur HTTP local récupère le \texttt{code} d'autorisation retourné par Gitlab, puis échange ce code contre un \textit{token} d'accès API. Cette opération est encapsulée dans la méthode suivante :
...
\begin{itemize}
    ...
    \item Fichier base de données : \texttt{blockchain.db}
    \item Clé privée : \texttt{private-key.pem}
    \item URL API Gitlab : \texttt{https://gitlab.cri.epita.fr/api/v4}
\end{itemize}
...
Pour améliorer l’ergonomie, la méthode \texttt{ask} permet de poser des questions à l’utilisateur avec une gestion simple des retours clavier. Un validateur de login simple est aussi inclus :

\begin{lstlisting}[language=Rust]
fn _check_login_syntax(login: String) -> bool
\end{lstlisting}

Ce système permet de garantir que l'identifiant utilisateur respecte bien le format \texttt{prenom.nom} imposé par l'infrastructure Gitlab de l'Épita.
...
Nexium nous permet d'explorer des concepts cryptographiques avancés, tel que la \textbf{blockchain}, l'\textbf{architecture réseau} et le \textbf{développement web}. 

En appliquant des principes de sécurité éprouvés comme le \textbf{SHA-256} pour le hachage et le chiffrement \textbf{RSA} pour l'authentification, nous avons pu établir les bases d'un système robuste et fiable que nous comptons concrétiser le plus fidèlement dans les prochaines étapes du développement.

Nous avons mis en place au fil de cette première étape du projet les fondations essentielles de la \textbf{structure théorique} et de la \textbf{cryptographie}, en s'inspirant du Bitcoin et en l'adaptant aux besoins spécifiques dans notre cas d'usage. 

Le \textbf{site web} et la \textbf{documentation} permettront une transparence et une accessibilité accrues pour nos travaux.

Nexium, au-delà d'être un exercice technique, est une expérimentation réelle des enjeux modernes liés à la \textbf{décentralisation} et à la \textbf{souveraineté numérique}, autant de thématiques qui nous paraissent importantes et que nous approfondissons en nous questionnant sur l’impact des cryptomonnaies dans notre société.

Les prochaines étapes du projet seront tout aussi importantes : \textbf{optimisation des performances}, \textbf{sécurisation structurelle du réseau} et \textbf{interface utilisateur intuitive} sont dans notre ligne de mire.


\newpage

% use \nocite{*} to include all entries in the bibliography
\nocite{*}
